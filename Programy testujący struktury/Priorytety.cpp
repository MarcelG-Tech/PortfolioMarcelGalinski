...
Node::Node(Element d) : data(d), prev(nullptr), next(nullptr) {}


// Konstruktor
KopiecMax::KopiecMax(int poczatkowaPojemnosc) {
...
bool KopiecMax::czyWyzszyPriorytet(
    const Element& dziecko,
    const Element& rodzic
) {
...
void KopiecMax::powiekszTablice() {
...
void KopiecMax::heapifyUp(int index) {

    while (index > 0) {

        int parentIndex = (index - 1) / 2; // wzór na rodzica

        if (czyWyzszyPriorytet(
                tablica[index],
                tablica[parentIndex])) {
...
void KopiecMax::heapifyDown(int index) {
...
// peek() / find-max()
Element KopiecMax::peek() {

    if (rozmiar <= 0) {
        throw out_of_range("Kopiec jest pusty!");
    }

    // Złożoność idealne O(1) - korzeń zawsze ma najwyższy priorytet
    return tablica[0];
}


// return-size() - Zwraca aktualny rozmiar kolejki
int KopiecMax::returnSize() {
...
void KopiecMax::modifyKey(
    int wartoscElementu,
    int nowyPriorytet
) {

    // W standardowym kopcu nie wiemy, gdzie leży element,
    // więc musimy go znaleźć (O(n))
    int index = -1;
...
// insert(e, p) wpisuje wartość i priorytet
void ListaDwukierunkowa::insert(
    int wartosc,
    int priorytet
) {
...
Element ListaDwukierunkowa::extractMax() {
    ...
    while (current != nullptr) {
        ...
        // Używamy TYLKO znaku '>',
        // więc przy remisie zostaje starszy element (FIFO)
        if (current->data.priority >
            maxNode->data.priority) {
...
Element ListaDwukierunkowa::peek() {
...
int ListaDwukierunkowa::returnSize() {
...
void ListaDwukierunkowa::modifyKey(
    int wartoscElementu,
    int nowyPriorytet
) {
    ...
    while (current != nullptr) {
        ...
        if (current->data.value == wartoscElementu) {
            ...
            // Lista pozostaje nieposortowana,
            // więc niczego nie musimy przestawiać
            return;
        }
...
int main() {
    ...
    uniform_int_distribution<> dystrybucjaPriorytetu(
        1,
        500000
    );

    uniform_int_distribution<> dystrybucjaWartosci(
        1,
        10000
    );
    ...
    cout << "Parametry: Probki = "
         << PROBKI
         << " na kazde N"
         << endl;

    cout << "N;Kopiec_Max(us);Lista_Dwukierunkowa(us)"
         << endl;
    ...
    for (int N = N_MIN; N <= N_MAX; N += KROK) {
        ...
        for (int p = 0; p < PROBKI; p++) {
            ...
            for (int i = 0; i < N; i++) {
                ...
                el.value =
                    dystrybucjaWartosci(gen);

                el.priority =
                    dystrybucjaPriorytetu(gen);
            ...
            for (int i = 0; i < N; i++) {
                ...
                kopiec.insert(
                    daneWzorcowe[i].value,
                    daneWzorcowe[i].priority
                );
        ...
        double sredniaKopiec =
            sumaCzasKopiec / PROBKI;

        double sredniaLista =
            sumaCzasLista / PROBKI;
